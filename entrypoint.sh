...
CHECKDIR=$(diff <(find . -maxdepth 2 -type d | grep -v .git |sort) <(curl -s ${PROJECT_LAYOUT}))
...
CHECKNAMES=$(find . -not -path './.git/*' -not -wholename '[a-z\._/0-9]*' | grep -i -v -E "README|MANIFEST|Snakefile|Makefile|LICENSE|Snakejob" )
